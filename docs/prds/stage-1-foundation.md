...
- **Relationship to Stage 0**: Stage 0 shipped the infra baseline. Stage 1 **preserves all of it**. `auth.py` is widened in place to add ES256/JWKS validation alongside HS256 (no module replacement). `realtime_outbox` stays as the `NotificationService.send()` Realtime broadcast fallback. `user_push_subscriptions`, `notification_preferences`, device/preferences routers, and the 3-channel notification service all carry over unchanged. No Stage 0 code is removed. Stage 0b (originally scoped as a separate deploy stage) is subsumed by Stage 1 — there is no standalone deploy stage.
